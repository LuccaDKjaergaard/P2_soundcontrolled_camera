...
//const uint8_t mainMAC[6] = {12, 212, 218, 90, 194, 148};
const uint8_t slaveMAC[6] = {48, 174, 164, 123, 70, 204};
...
struct peerInfoTp {
  ...
  struct wirelessDataTp receivedData;
  long unsigned lastSeenTime;
};
struct peerInfoTp slave;
...
void InitESPNOW() {
  ...
  //connect to peer:
  for(int i = 0; i < 6; i++) {
    slave.info.peer_addr[i] = slaveMAC[i];
  }
  slave.info.channel = 0; //use default channel
  slave.info.encrypt = false; //can be changed later

  if(esp_now_add_peer(&slave.info) == ESP_OK) {
    Serial.println("Peer added successfully.");
  } else {
    Serial.print("Failed to add peer. Error code: ");
    Serial.println(esp_now_add_peer(&slave.info));
  }
}

void OnDataReceived(const uint8_t* mac, const uint8_t* incomingData, int length) {
  memcpy(&slave.receivedData, incomingData, sizeof(slave.receivedData));
  //update lastSeenTime
}
